...
function init() {
	...
	manager.onLoad = function ( ) {
		...
		if(currState === POOL){
			//TODO IF RESET FUNCTION IS WORKING UNCOMMENT
			// controls.reset()
			// skyBox.rotation.y = 2
			// scene.rotation.y =2
...
function animate() {
	...
	if(currState === SELFIE){
		...
		if(selfieLoad == true && dirVector.z > 0.25 && dirVector.z < 0.75 && dirVector.y > -0.3 && dirVector.x < 0 ){
		...
		if(poolLoad == true && hoverButtonChecker === false && dirVector.z > -0.4 && dirVector.z < 0.9 && dirVector.y > -0.3 && dirVector.x > 0 && dirVector.x < 1   ){ 
		...
		// console.log(dirVector.x +', '+dirVector.y +', '+dirVector.z);
		if(hoverButtonChecker == false && dirVector.z > -0.8 && dirVector.z < 0.8 && dirVector.y > -0.3 && dirVector.x < 0 ){
