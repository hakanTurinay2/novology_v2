...
function init() {
	...
	manager.onLoad = function ( ) {
		...
		if(currState === INTRO){
			skyBox.rotation.y = 0.55
			scene.rotation.y =0.55
			...
			if(selectedLanguage == "ko"){
				textMainRoom.position.set(arrowDist * Math.sin(toRadians(0)) , arrowHeight*1.225, -arrowDist * Math.cos(toRadians(0)));
				textMiddleRoom.scale.set(18,18,18)
				textMainRoom.scale.set(12,12,12)
			}else if(selectedLanguage == "zh"){
				textMainRoom.position.set(arrowDist * Math.sin(toRadians(0)) , arrowHeight*1.18, -arrowDist * Math.cos(toRadians(0)));
				textMainRoom.scale.set(10,10,10)
				textMiddleRoom.scale.set(13,13,13)
			}else{
				textMainRoom.position.set(arrowDist * Math.sin(toRadians(0)) , arrowHeight*1.225, -arrowDist * Math.cos(toRadians(0)));
		...
		if(currState === SELFIE){
			controls.reset()
			skyBox.rotation.y = -2.25
			scene.rotation.y =-2.25
			video.currentTime = 0;
			video2.currentTime = 0;
			...
			textPool.position.set(1.65*arrowDist * Math.sin(toRadians(65)) , arrowHeight*1.2, -arrowDist *1.65* Math.cos(toRadians(65)));
			
			if(selectedLanguage == "ko"){
				textPool.scale.set(22,22,22)
			}else if(selectedLanguage == "zh"){
				textPool.scale.set(22,22,22)
			}else{
				textPool.scale.set(18,18,18)
			}
		...
		if(currState === POOLENTRACE){
			// controls.reset()
			skyBox.rotation.y = -1.83
			scene.rotation.y =-1.83
			// skyBox.rotation.y = -1
			// scene.rotation.y =-1
			video.currentTime = 0;
			video3.currentTime = 0;
			...
			textMainRoom.position.set(arrowDist * Math.sin(toRadians(190)) , arrowHeight*1.225, -arrowDist * Math.cos(toRadians(190)));
			...
			if(selectedLanguage == "ko"){
				...
				textSelfie.scale.set(25,25,25)
				textPool.scale.set(35,35,35)
			}else{
				textSelfie.scale.set(20,20,20)
				textPool.scale.set(28,28,28)
				textMiddleRoom.scale.set(12,12,12)
			}
		...
		if(currState === MAIN){
			...
			scene.rotation.y =-1.7
		
			videoManager.itemStart(orbVideoUrl);
			// videoManager.itemStart(leadingURL+"video/orb/Orb.Alpha.V8.webm");
			video.currentTime = 0;
			video3.currentTime = 0;
			...
			textPoolEntrance.position.set(1.2*arrowDist * Math.sin(toRadians(15)) , arrowHeight*1.225, -arrowDist*1.2 * Math.cos(toRadians(15)))
...
function animate() {
	...
	if(currState === SELFIE){
		...
		if(selfieLoad == true &&  dirVector.z < -0.6 && dirVector.y > -0.3 && dirVector.x < 0.6 && dirVector.x > -0.6 ){ // need to stress test
		...
		if( poolLoad == true && hoverButtonChecker == false && dirVector.z < -0.6 && dirVector.y > -0.3 && dirVector.x > -0.6 && dirVector.x < 0.6   ){ // need to stress test
		...
		if(hoverButtonChecker == false &&  dirVector.z < -0.5 &&  dirVector.y > -0.3  &&  dirVector.y < 0.3  && dirVector.x < 0.6 && dirVector.x > -0.6 ){ // need to stress test
