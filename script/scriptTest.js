...
function init() {
	...
	sceneManager.onLoad = function () {
		...
		if(currentState === INTRO){
			...
			skyBox.rotation.y = 0
			scene.rotation.y =0
			var newPos = new THREE.Vector3(-20,-20,-20)
			Scene1Video.add(VideoPlayBottleScene)
			VideoPlayBottleScene.position.set(newPos);
			// console.log("World: " + localToWorld(newPos))
			// VideoPlayBottleScene.applyMatrix()
			console.log("Local: " + VideoPlayBottleScene.worldToLocal())
		...
		else if(currentState === PIGMENTATION){
			controls.reset();
			skyBox.rotation.y = 0
			scene.rotation.y =0
